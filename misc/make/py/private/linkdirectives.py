...
from make.py.private.exceptionclasses import SyntaxError, LogicError
...
class LinkDirectives(object):
    ...
    def error_check(self):
        ...
        if not re.search('\*', self.linkedfile) and re.search('\*',
                                                              self.localfile):
            raise SyntaxError(messages.syn_error_wildfilename1.replace('\n',
                              ''))
        if re.search('\*', self.localfile) and not re.search('\*$',
                                                             self.localfile):
    ...
    def create_flag_list(self):
        ...
        if re.search('\*.+', self.linkedfile) \
                or re.search('^.+\*', self.linkedfile) \
                or (re.search('\*', self.localfile)):
            # Make wildcard list
            self.flag_list = True
    ...
    def command(self, logfile, quiet):
        ...
        if os.name == 'posix':
            ...
            options = (self.linkeddir, self.linkedfile, self.localdir,
                       self.localfile)
        else:
            if self.linkedfile == '':
                option = '/d'
            ...
            options = (option, self.localdir, self.localfile, self.linkeddir,
                       self.linkedfile)
        ...
        print(messages.success_makelink % (self.linkeddir,
                                           self.linkedfile,
                                           self.localdir,
                                           self.localfile), file=logfile)
    ...
    def add_to_dict(self, links_dict):
        if self.flag_list:
            for element in self.LIST:
                ...
                local = os.path.relpath(os.path.join(self.localdir,
                                        self.localfile))
