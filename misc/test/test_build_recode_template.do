...
quietly {
    ...
    program test_basic
        ...
        import delimited using temp/recode1.csv, clear
        describe
        assert r(k) == 5
    ...
    program test_keyed
        ...
        build_recode_template using temp/main.dta, output(temp/recode2.csv) variables(attribute1 attribute2 attribute3) ///
            key(key)
        import delimited using temp/recode2.csv, clear
        describe
        assert r(k) == 6
    ...
    program test_descriptors
        ...
        import delimited using temp/recode3.csv, clear
        describe
        assert r(k) == 5
        capture confirm variable attribute code description recode notes
        assert !_rc
    ...
    program setup_import_dataset
        preserve
        import delimited using temp/recode2.csv
        replace recode = 1 if _n<=10
        ren recode newcode
        drop notes
        gen notes = "newcode" if _n<=10
        export delimited using temp/recode4.csv, replace
        restore
    end
    ...
    program test_import
        ...
        import delimited using temp/recode5.csv, clear
        describe
        assert r(k) == 6
    ...
    program test_import_extended
        ...
        build_recode_template using temp/main.dta, output(temp/recode6.csv) variables(attribute1 attribute2 attribute3) ///
            key(key) import(temp/recode4.csv) recode_var_name(newcode)
        import delimited using temp/recode6.csv, clear
        describe
        assert r(k) == 6
        assert r(N) == 249
        capture confirm variable key attribute code description newcode notes
    ...
    program test_quantity_and_count
        ...
        import delimited using temp/recode9.csv, clear
        describe
