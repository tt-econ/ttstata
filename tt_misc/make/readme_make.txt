...
Prequisites:
(1)  Python 3.5 installed and executable path is added to system path
...
(6)  StatTransfer installed and executable path is added to system path (optional)
(7)  LyX installed and executable path is added to system path
(8)  R installed and executable path is added to system path
(9)  SAS installed and executable path is added to system path (optional)
(10) Lalexmk package installed (default in TeXLive after 2009) and updated to a 2013 and after distribution
...
Default options are defined in /private/metadata -> default_options.
...
* clear_output_dirs([output_dir[, temp_dir]]):
    """Create "output_dir" and "temp_dir" directories if they do not already exist.
    Delete all files in output and temp directories keeping directory structure
    (i.e. sub-directories).

    If output_dir == '' then output directory will not be created.
    If temp_dir == '' then temp directory will not be created."""

* clear_output_dirs_local([output_dir[, temp_dir[, output_local_dir]]])
    Creates "output_local_dir" directory if it does not already exist.
    Delete all files in the directory keeping directory structure
    (i.e. sub-directories). Then calls clear_output_dirs.

    If output_local_dir == '' then output local directory will not be created.
...
pdfonly = bool_pdfonly              (optional, only for run_tex, default = True)
...
using run_lyx().  If unspecified, run_lyx('file.lyx') produces 'file.pdf' in the same directory
as the lyx file.

* [bool_pdfonly] is a Boolean (True/False) to specify whether we want to keep only the pdf output
or also all the auxiliary files created by latexmk (pdflatex + bibtex, etc.).
...
- To delete all non-hidden files in ../output and all of its non-hidden sub-directories:
    delete_all_files('../output')

- To delete all non-hidden files in ../output except for any file named 'make.log':
    delete_all_files('../output', '../output/make.log')

    Note that the path specified in the second argument is irrelevant, only the file name will be
considered.
...
- This is usually put at the start of make.py before start_make_logging(). To create the default
'../output' directory and the default '../temp' directory if not yet created, and to delete all files in
'../output' and '../temp' keeping directory structure (i.e. sub-directories):
    start_make_logging()

* clear_output_dirs_local

- This is used in the same way as clear_output_dirs(), except it should be used only in
  svn_derived_local directories.
...
- This function can be used to overwrite the default setting options. All other functions that come
after it will not have to re-specify those options.
...
- To create pdf file for 'draft.lyx' with the log file being './make.log':
    ...
    By default, the pdf file is saved in the same directory as the lyx file, and has the same name as the lyx file.

    e.g. if set_option(output_dir = '../output'),
    then run_lyx(program = 'draft') produces 'draft.pdf'.
...
- To run PdfLaTeX to convert 'file.tex' to 'file.pdf' and the common log
file is saved in string variable makelog_file:

    run_tex(program = 'file', makelog = makelog_file)

    This will execute and process the log file, and delete aux files:
    latexmk -pdf -ps- -f -quiet -output-directory=temp file.tex

    file.pdf will then be moved from /temp and /temp is deleted to delete all aux files.

    To disable this feature, run the following command:

    run_tex(program = 'file', pdfonly = False)
    This will execute and process the log file:
    latexmk -pdf -ps- -f -quiet -output-directory=[current_path_of file.tex] file.tex

- Note that changedir option is always set to True for this function.

- Note that all previous output files are deleted before latexmk is run.


* run_command

- This function is to faciliate any other Shell command that has not yet been implemented as a
stand-alone function. To run the command 'wzzip -a -P -r ../output/ttecon_data.zip ./*.*',
with main log file 'make.log' and log file 'zip.log':

    run_command(command = 'wzzip -a -P -r ../output/ttecon_data.zip ./*.*', makelog = 'make.log', log = 'zip.log')
