...
from make.py.private.exceptionclasses import CustomError, CritError
...
def start_logging(log, logtype):
    ...
    print(messages.note_logstart % logtype, time_begin,
          working_dir, file=LOGFILE)
    LOGFILE.flush()
...
def input_to_array(filename):
    ...
    for line in FILENAME:
        if (not re.match('rev', line) and not re.match('linkpath', line) and
