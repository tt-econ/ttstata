...
from make.py.make_link_logs import make_link_logs
...
class testMakeLinkLogs(unittest.TestCase):
    ...
    def setUp(self):
        ...
        files = [f for f in files if (os.path.isfile(
                                      os.path.join(self.test_dir, f)) and not
                                      f.startswith('.'))]
    ...
    def test_default_log(self):
        ...
        links_data = open('./input/links_simple.txt', 'r').readlines()
        comments = self.find_comment_lines(links_data)
        number_of_links = len(links_data) - comments
        logged_links = len(open(default_orig, 'r').readlines()) - 1
